@file:UseSerializers(LocalDateTimeSerializer::class, MoneySerializer::class)
...
@Serializable
@SerialName("win_count_condition")
class WinCountCondition(
    val betType: BetType,
    val betBonusType: BetBonusType,
    val matching: Matching,
    val includeCurrentPeriod: Boolean? = null,
    val period: Period? = null,
    @Contextual val date: Range<LocalDateTime>? = null,
    @Contextual val count: Range<Int>? = null,
    ...
    override suspend fun validate(): List<Error> {
        return validate(this) {
            if (date != null) validate(WinCountCondition::date).isValidRange()
            if (period != null) validate(WinCountCondition::includeCurrentPeriod).isNotNull()
            if (currencyCounts == null) validate(WinCountCondition::count).isNotNull().isValidRange()
            if (count == null) {
                validate(WinCountCondition::currencyCounts).isNotNull().isNotEmpty().validate {
    ...
    private fun casinoQuery(count: Range<Int>, currency: Currency? = null): Query {
        ...
        return CasinoBetResultStatisticTable
            ...
            .select { CasinoBetResultStatisticTable.winAmount neq 0 }
            .apply {
    ...
    private fun Query.filterCount(count: Range<Int>): Query {
        return when {
            count.from != null && count.to != null -> having {
                CasinoBetResultStatisticTable.winAmount.count() greaterEq count.from and
                    (CasinoBetResultStatisticTable.winAmount.count() lessEq count.to)
            }
            count.from != null -> having { CasinoBetResultStatisticTable.winAmount.count() greaterEq count.from }
            count.to != null -> having { CasinoBetResultStatisticTable.winAmount.count() lessEq count.to }
