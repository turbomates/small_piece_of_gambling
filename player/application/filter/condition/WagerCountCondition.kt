...
@Serializable
@SerialName("wager_count_condition")
class WagerCountCondition(
    val betType: BetType,
    ...
    override suspend fun validate(): List<Error> {
        return validate(this) {
            if (date != null) validate(WagerCountCondition::date).isValidRange()
            if (period != null) validate(WagerCountCondition::includeCurrentPeriod).isNotNull()
            if (currencyCounts == null) validate(WagerCountCondition::count).isNotNull().isValidRange()
            if (count == null) {
                validate(WagerCountCondition::currencyCounts).isNotNull().isNotEmpty().validate {
    ...
    private fun sportsbookStrictIds() = emptySet<UUID>()
