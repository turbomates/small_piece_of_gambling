...
@Serializable
@SerialName("loss_sum_condition")
class LossSumCondition constructor(
    val betType: BetType,
    ...
    override suspend fun validate(): List<Error> {
        return ktorValidate(this) {
            validate(LossSumCondition::balances).isNotEmpty().isValidMoneyRange()
            if (date != null) validate(LossSumCondition::date).isValidRange()
            if (period != null) validate(LossSumCondition::includeCurrentPeriod).isNotNull()
        }
    }
    ...
    private fun casinoQuery(dateRange: Range<LocalDateTime>?, balanceRange: Range<Money>): Query {
        ...
        return CasinoBetResultStatisticTable
            ...
            .andWhere { CasinoBetResultStatisticTable.winAmount eq 0 }
            .filterBet()
            .apply {
                casino?.apply {
    ...
    private fun Query.filterSum(balanceRange: Range<Money>): Query {
        return when {
            ...
            balanceRange.from != null -> having { CasinoBetResultStatisticTable.betAmount.sum() greaterEq balanceRange.from.amount }
            balanceRange.to != null -> having { CasinoBetResultStatisticTable.betAmount.sum() lessEq balanceRange.to.amount }
